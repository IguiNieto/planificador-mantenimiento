...
// NOTA TEMPORAL DE DIAGNÓSTICO: mientras resolvemos el guardado, mostramos
// el error real en un alert() para poder verlo sin DevTools. Se saca
// apenas quede confirmado el problema.
function mostrarErrorDiagnostico(origen, mensaje) {
  if (typeof window !== "undefined") {
    window.alert(`[Diagnóstico ${origen}]\n${mensaje}`);
  }
}
  ...
  async get(key) {
    try {
      ...
      if (error) {
        ...
        mostrarErrorDiagnostico("get:" + key, error.message);
        return null;
      }
      if (!data) return null;
      ...
      mostrarErrorDiagnostico("get-exception:" + key, e.message || String(e));
      return null;
    }
  },
  ...
  async set(key, value) {
    try {
      ...
      if (error) {
        ...
        mostrarErrorDiagnostico("set:" + key, error.message);
        return null;
      }
      return { key, value, shared: true };
    } catch (e) {
      console.error("storage.set exception:", key, e);
      mostrarErrorDiagnostico("set-exception:" + key, e.message || String(e));
      return null;
    }
  },
};
